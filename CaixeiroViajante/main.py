...
with open('29_cidades.txt', 'r') as file:
    linhas = file.readlines()
    
cidades_percorridas = []
distancias_percorridas = []
quantidade_de_cidades = len(linhas)
cidades = []

contador = 0
for linha in linhas:
    distancias = linha.split()
    distancias_formatadas = []
    for distancia in distancias:
        distancias_formatadas.append(int(distancia))
    
    cidades.append(Cidade(contador, distancias_formatadas))
    contador = contador + 1
...
while len(cidades_percorridas) < quantidade_de_cidades + 1:
